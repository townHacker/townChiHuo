(function($) {
    $.fn.extend({
        loading: function() {
            var container = $(this);
            var layer = $('<div class="loading-layer">');
            var loading_img = $('<img>');
            loading_img.attr('src', '/images/icons/loading/loading16.gif');
            
            var c_w = container.width();
            var c_h = container.height();
            layer.append(loading_img);
            layer.css({
                'width': c_w,
                'height': c_h,
                'position': 'absolute',
        ...
        unloading: function() {
            var container = $(this);
            container.children('.loading-layer').each(function() {
                $(this).remove();
            });
        }, 
        
        figure_cut_cancel: function() {
            var container = $(this);
            container.empty();
            var img = $('<img src="/js/holder.js/400x400" />');
            container.append(img);
        },
        
	    figure_cut: function(option) {
	        option = $.extend({
                preview_btn: '#btn_figure_preview',
                enter_btn: '#btn_figure_enter',
	            slider: '#figure_cut_slider',
	            zoom_val: '#zoom_val',
                img_src: $(this).attr('img-src'),
	        }, option);
	        var container = $(this);
